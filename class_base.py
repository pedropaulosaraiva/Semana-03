...
# Métodos Novos
    ...
    def getCirAT_MT_DB(self, nomeSE_AT):
        ...
        try:
            ctats = self.DataBaseConn.getSQLDB("CTAT", "SELECT DISTINCT nom FROM ctat WHERE nom LIKE '" +
                                               nomeSE_AT + "%' AND NOT (nom LIKE '%2' OR nom LIKE '%3' OR nom LIKE \
                                               '%4') ORDER BY nom")
    ...
    def getSE_MT_AL_DB(self, nomeSE_MT):
        ...
        try:
            ctmts = self.DataBaseConn.getSQLDB("CTMT", "SELECT DISTINCT nom, cod_id FROM ctmt WHERE sub\
                                               = '" + nomeSE_MT[0]+"'ORDER BY nom")  # OBS: nomeSE_MT é uma lista
    ...
    def getSE_MT_AL_TrafoDIST(self, codField):
        ...
        try:
            untrds = self.DataBaseConn.getSQLDB("UNTRD", "SELECT DISTINCT cod_id, pot_nom FROM  untrd\
